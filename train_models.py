...
# Define common numerical features
numerical_cols = ['Soil pH', 'Nitrogen', 'Phosphorus', 'Potassium', 'Temperature', 'Humidity', 'Rainfall']
# Define categorical features that need encoding
# 'Crop Type' will be encoded as it's a target for one model and an input for another
categorical_cols_to_encode = ['Crop Type', 'Fertilizer Name']
...
if missing_cols:
    ...
    print("Please ensure your 'Fertilizer.csv' has all the necessary columns for both crop and fertilizer predictions.")
    exit(1)
...
# Features for Crop Recommendation: Soil pH, N, P, K, Temperature, Humidity, Rainfall
# Target: Crop Type_encoded
crop_features_df = df[numerical_cols]
...
# This model only needs its own encoder for 'Crop Type' to decode predictions
...
# Features for Fertilizer Recommendation: Encoded Crop Type, Soil pH, N, P, K, Temperature, Humidity, Rainfall
# Target: Fertilizer Name_encoded
fertilizer_features_df = df[['Crop Type_encoded'] + numerical_cols]
...
# This model needs both 'Fertilizer Name' and 'Crop Type' encoders
# because 'Crop Type' is an input feature that needs to be encoded before prediction
fertilizer_model_data_to_save = {
    'model': fertilizer_model,
    'label_encoders': {'Fertilizer Name': le_fertilizer, 'Crop Type': le_crop},
